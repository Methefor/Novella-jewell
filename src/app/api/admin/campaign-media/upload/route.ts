...
const payloadSchema = z.object({
  ...
  productIds: z.array(z.string().min(1).max(180)).max(3),
  ...
  try {
    const result = await handleUpload({
      ...
      onBeforeGenerateToken: async (pathname, clientPayload) => {
        ...
        const client = z.object({ campaignId: z.string().uuid(), format: z.enum(['story', 'feed', 'square']), filename: z.string().max(240), size: z.number().int().positive().max(60 * 1024 * 1024), productIds: z.array(z.string().min(1).max(180)).max(3) }).parse(JSON.parse(clientPayload ?? '{}'));
