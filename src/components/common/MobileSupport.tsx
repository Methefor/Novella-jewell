...
export default function MobileSupport() {
  ...
  return <aside aria-label="Hızlı destek" className="fixed right-4 z-30 flex rounded-full border border-black/10 bg-white shadow-lg lg:hidden bottom-[calc(env(safe-area-inset-bottom)+5.5rem)]">
    <a href={`https://wa.me/${SITE.whatsapp}`} target="_blank" rel="noopener noreferrer" className="flex h-12 items-center gap-2 rounded-full bg-[#16130f] px-4 text-xs font-medium text-white focus-visible:ring-2 focus-visible:ring-black"><MessageCircle className="h-4 w-4" />WhatsApp destek</a>
  </aside>;
}
