    ...
    <section ref={ref} className="relative min-h-[760px] overflow-hidden bg-[#f1ece3] lg:min-h-[calc(100dvh-var(--navbar-h))]" aria-label="Novella yeni yüzük vitrini">
      ...
      <div className="container-custom relative grid min-h-[760px] items-center gap-10 pb-12 pt-12 lg:min-h-[calc(100dvh-var(--navbar-h))] lg:grid-cols-[.82fr_1.18fr] lg:pb-10 lg:pt-8">
        <motion.div style={{ y: reduceMotion ? undefined : copyY, opacity: reduceMotion ? 1 : copyOpacity }} className="relative z-20 max-w-[610px] pt-2 lg:pt-0">
          <motion.div initial={false} animate={{ opacity: 1, y: 0 }} className="mb-6 flex items-center gap-3">
          ...
          <h2 className="font-editorial text-[#16130f]" style={{ fontSize: 'clamp(3.35rem, 7.2vw, 7.6rem)', lineHeight: 0.86, letterSpacing: '-0.045em' }}>
            <RisingLine delay={0.2}>Özgün</RisingLine>
            <RisingLine delay={0.34} className="italic text-[#75603b]">parçalar.</RisingLine>
            <span className="mt-2 block font-serif text-[.56em] leading-[.98] tracking-[-.035em]">Ulaşılabilir bir lüks.</span>
          </h2>
          <p className="mt-7 max-w-md text-[14px] font-light leading-7 text-black/65 md:text-[15px]">
          ...
          <motion.div initial={false} animate={{ opacity: 1, y: 0 }} className="mt-8 flex flex-wrap items-center gap-5">
...
function RisingLine({ children, delay, className = '' }: { children: React.ReactNode; delay: number; className?: string }) {
  return <span className={`block overflow-hidden pb-[.08em] ${className}`}><motion.span className="block" initial={false} animate={{ y: 0 }} transition={{ duration: 1.05, delay, ease }}>{children}</motion.span></span>;
}
